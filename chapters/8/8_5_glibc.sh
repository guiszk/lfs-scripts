...
time make # 17m15.612s
time make check # 69m53.273s
# You may see some test failures. The Glibc test suite is somewhat 
# dependent on the host system. A few failures out of over 4200 
# tests can generally be ignored. This is a list of the most common 
...
#    - The stdlib/tst-arc4random-thread test is known to fail if the host kernel 
#      is relatively old.
#    - Some tests, for example nss/tst-nss-files-hosts-multi, are known to fail 
#      on relatively slow systems due to an internal timeout.
